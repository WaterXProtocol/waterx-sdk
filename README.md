...
**No cross-source fallback, no feeds guard anywhere.** Construction rejects an empty/nullish `oracleSource` **and any value outside `ORACLE_SOURCES`** (a legacy `'core'` / `'pyth'` / `'pyth_rule'` string fails at `create`). Beyond that, the fed set names what the client is **willing** to push and the loaded config decides what it **can**: a listed source the deployment has no package block for — or whose `feeds` map is empty — simply contributes nothing, at init _and_ at tx-build. A requested ticker that **no selected source produced a leg for** is **skipped**, not fatal: `refreshOraclePrices` returns `{ refreshed, skipped }` and the PTB carries the legs the deployment actually has (a ticker is servable with no update leg only when `constant_rule` is the sole rule wired for it). Note `skipped` has two indistinguishable causes — the config wires no ready rule, **or** the ticker is wired only under a rule this client's `oracleSource` omits (a fed-set mismatch, not malformed config).

A skipped ticker keeps whatever price the on-chain `Oracle` already holds, and **the chain will not necessarily reject it** — the stored price can still be inside the freshness window, and on the WLP path `update_token_value` re-stamps `last_price_refresh_timestamp` off that stored price, defeating `assert_prices_fresh` outright. So the high-level `build*Tx` composers **fail closed** with `OracleTickerUnservedError`:

| builder                                                     | action-critical set                                                                                                                                                                                                                   |
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| trading (`buildPlaceOrderTx`, position/collateral builders) | traded market + its collateral                                                                                                                                                                                                        |
| WLP mint / mint+stake / unstake+redeem                      | **every pool asset** — `mint_wlp` takes `&WlpAum` and sizes the payout against `pool.tvl_usd`, so any stale pool price mis-mints. Also rejects a pool asset the config wires no ready rule for, which never reaches `skipped` at all. |

Pass `allowUnrefreshedPrices: true` to accept the on-chain price deliberately. A present-but-wrong feed id is likewise not validated by the SDK; it aborts on-chain at dry-run.
...
| `OracleTickerUnservedError: no listed oracle source […] served ticker(s) …`   | A `build*Tx` refused to append an action against a ticker that got no oracle leg. Fix the fed set / config, or pass `allowUnrefreshedPrices: true` to execute at the price already on chain.                                                                                                                                                                                                                                                                                                              |
...
| A ticker appears in `refreshOraclePrices`'s returned `skipped` list           | No **selected** source produced a leg for it — either the config wires no ready rule, or it is wired only under a source your `oracleSource` omits. Check both before assuming the config is at fault. Add the feed under a listed source, or list a source that serves it. Constant-only tickers are never skipped.                                                                                                                                                                                      |
