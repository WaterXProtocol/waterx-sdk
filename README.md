...
| -------------- | ---------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `oracleSource` | `OracleSource \| OracleSource[]` of `'pyth_lazer_rule'` \| `'waterx_rule'` — REQUIRED, no default | The price-update source(s). `'pyth_lazer_rule'` = Pyth Lazer signed updates (ONE `leEcdsa` verify per PTB, no per-feed fees); needs `packages.pyth_lazer_rule` feeds + a `pythApiKey` (auth-first). `'waterx_rule'` = the first-party WaterX quote-center (Nautilus-TEE, ed25519-signed CEX prices): one signed Merkle leaf per ticker (batch-envelope fallback), no API key and no per-update fee; needs `packages.waterx_rule` feeds. (`'pyth_rule'` — Pyth Core / Hermes — was RETIRED in 5.0.0; `parseOracleSourceList` rejects it with a retired-value hint.) |
...
This is the coexistence rollout pattern: staging lists every source under migration (`ORACLE_SOURCE=waterx_rule,pyth_lazer_rule` + `PYTH_API_KEY`) while production trails with its own list until its weight tables move — flipping an environment is an env-var change, never an SDK release.
...
1. **Implement `PriceUpdateRule`** in `src/oracle/rules/<name>-rule.ts` — all port fields (`src/oracle/price-update-rule.ts`): `kind`, `requiredCredential` (set iff the off-chain fetch needs a caller credential — gates the fail-fast credential pre-check and consumers' `missingOracleCredentials` boot asserts), `supportedTickers`, `fetchUpdateData`, `narrowUpdateData` (subset a cached whole-universe payload to one build's tickers — a divisible payload returns a per-feed subset, an indivisible one returns itself whole iff fully covered; uncovered ticker → `null` miss), `updateIdentityBySymbol` (iff the on-chain verify is replay-guarded per symbol), `buildUpdateCalls`.
...
| `oracleSource is REQUIRED and must name at least one of …` | Missing, empty, or a retired value (`'core'`, `'pyth'`, and since 5.0.0 `'pyth_rule'`). Parse env with `parseOracleSourceList`, not a bare `split`. |
