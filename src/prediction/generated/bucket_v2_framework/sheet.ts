...
export const Sheet: MoveStruct<any, any> = new MoveStruct({ name: `${$moduleName}::Sheet<phantom CoinType, phantom SelfEntity>`, fields: {
        credits: vec_map.VecMap(Entity, liability.Credit),
        debts: vec_map.VecMap(Entity, liability.Debt),
        blacklist: vec_set.VecSet(Entity)
    } });
export const Loan: MoveStruct<any, any> = new MoveStruct({ name: `${$moduleName}::Loan<phantom CoinType, phantom Lender, phantom Receiver>`, fields: {
        balance: balance_1.Balance,
        debt: liability.Debt
    } });
export const Request: MoveStruct<any, any> = new MoveStruct({ name: `${$moduleName}::Request<phantom CoinType, phantom Collector>`, fields: {
        requirement: bcs.u64(),
        balance: balance_1.Balance,
