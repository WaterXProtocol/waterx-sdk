/**
 * Integer-domain guards for `perp/fetch` params (internal, not on the barrel).
 *
 * The generated BCS layer coerces `number` params only after JS arithmetic has
 * already lost precision — a non-safe integer (>= 2^53), fractional, or
 * negative value would serialize a silently-wrong integer into the PTB instead
 * of failing. Every u64/u128 param on the fetch surface funnels through these
 * so garbage throws with the parameter's name before a transaction is built.
 */
