 ...
 * first-party quote-center ed25519 signed batches). Pyth Core (`pyth_rule`)
 * was removed outright — it is not an `OracleSource` any more, so a stale
 * `ORACLE_SOURCE` naming it is rejected by `parseOracleSourceList` at the
 * consumer boundary. Resolving a source with no registered rule throws a
 ...
 * `instanceof`-able (mirrors `LazerApiKeyMissingError`)
