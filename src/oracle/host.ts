 ...
 * lookups plus the caller-supplied per-source access slices — so it depends on
 * this structural interface instead. `PerpClient` satisfies it without any `implements`
...
export interface OracleHost {
  ...
  /** Caller-supplied Pyth Lazer credential + fetch policy (create options) — NO endpoints, NO object ids. */
