 ...
 * boot instead.
 *
 * SCOPE — read this before relying on it. Each source serves exactly the
 * tickers its rule block lists (`oracle_rules.pyth_lazer.lazer_feed_ids`,
 * `oracle_rules.waterx.feeds`), so this catches a requested ticker that NO
 * listed source declares and that is not constant-pinned — e.g. a market the
 * document introduced as a symbol but wired to no rule. What it cannot catch
 * is a listed symbol the quote-center does not actually serve; that surfaces
 * at the first build's fetch instead. Verifying the on-chain half is
 * `assertOracleWeightCoverage` (`weight-coverage.ts`), which reads the
 * aggregators.
 */
