...
/**
 * `WATERX_CONFIG_URL` / `E2E_CONFIG_URL` → this harness's document URL.
 *
 * Same convention as `scripts/waterx-config-url.ts` (the canonical copy): the
 * env var is a CDN BASE root and `/<network>.json` is appended; a value ending
 * in `.json` is a legacy complete-file URL and is used as-is. Duplicated rather
 * than imported because this stress harness is a standalone workspace package
 * (`rootDir: src`) and cannot reach outside its own tree — keep the two in step.
 */
function resolveConfigUrl(raw: string | undefined): string | undefined {
  const value = raw?.trim();
  if (!value) return undefined;
  const path = value.split(/[?#]/, 1)[0] ?? value;
  // Stress runs are testnet-only.
  return /\.json$/i.test(path) ? value : `${value.replace(/\/+$/, "")}/testnet.json`;
}

/** Client options for the prediction e2e client. `loadConfig` never reads env —
 *  it only takes the `waterxConfigUrl` opt — so this harness composes the URL
 *  from `E2E_CONFIG_URL` (line-specific), falling back to the shared
 *  `WATERX_CONFIG_URL` that CI sets, and passes it as the opt. */
export function readE2eClientOverrides() {
  return {
    waterxConfigUrl: resolveConfigUrl(
