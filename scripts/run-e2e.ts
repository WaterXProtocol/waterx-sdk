 ...
 *
 * Examples:
 *   pnpm test:e2e                                 # mainnet (default)
 *   pnpm test:e2e --testnet                       # testnet
 *   pnpm test:e2e --mainnet -t "TC-TRADE-003"     # mainnet + test-name filter
...
function parseArgs(argv: string[]): { network: Network; forward: string[] } {
  ...
  const network: Network = cliNetwork ?? envNetwork ?? "mainnet";
