...
- **`tx-builders.ts`** — high-level async `build*Tx` wrappers that compose optional `appendConsolidateToUsd` pre-sweep (`consolidateToUsd`, default `true`), oracle refresh + `*Request` + `executeTrading` for perp trading, plus **`buildDepositFromWalletTx`** (wallet PSM mint). Sync low-level builders never auto-prepend the sweep — apps must call async `build*Tx` (or `buildConsolidateToUsdTx` separately).
