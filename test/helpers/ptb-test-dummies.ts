...
import { TESTNET_PACKAGE_IDS } from "../../src/constants.ts";
import type { CoinForReceiving } from "../../src/user/trading.ts";
...
/** TTO receiving vector used by `openPosition` / `placeOrder` / collateral adjust tests. */
export const PTB_DUMMY_RECEIVING_COINS: CoinForReceiving[] = [
  {
...
export function dummyBucketFloatPriceResult(
  ...
  const [a] = tx.moveCall({
    target: `${TESTNET_PACKAGE_IDS.BUCKET_FRAMEWORK}::float::from_scaled_val`,
