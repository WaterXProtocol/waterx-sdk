...
describe.skipIf(!hasWriteCredentials())("claim integration (sign + execute)", () => {
  ...
  it("resolveMarket + claim emit MarketResolved and PositionClaimed", async (testCtx) => {
    ...
    const resolveResult = await executeAndFetch(ctx.client, ctx.keeper, (tx) => {
      resolveMarket(ctx.client, tx, { marketId: bytes, outcome: "YES" });
    });
    ...
    const claimResult = await executeAndFetch(ctx.client, ctx.signer, (tx) => {
      claim(ctx.client, tx, { positionId: positionId! });
    });
