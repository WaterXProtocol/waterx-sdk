...
Off-chain oracle fetches (Pyth **Lazer** POST, WaterX **quote-center** GET) go through **`fetchWithPolicy`** (`src/oracle/update-fetch.ts`) — bounded retry with exponential backoff on network errors / 429 / 5xx (default: 2 retries) — and only throw once retries are exhausted. Pyth **Core / Hermes** is gone entirely (removed in 4.4.0), so `fetchPriceFeedsUpdateData` / `skipHermesIfFeedUnavailable` no longer exist.
...
- **Oracle:** **`refreshOraclePrices`** (`src/oracle/aggregate.ts`) against whichever source `ORACLE_SOURCE` names. There is **no** legacy bucket-aggregator prime step, and no Pyth Core update block.
...
| Command                  | Purpose                                                                                                                                                                                                                                                                                                                                      |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pnpm oracle:aggregates` | **mainnet** default; `pnpm oracle:aggregates:testnet` or `-- --testnet` for testnet. Lazer/quote-center refresh + oracle PTB **simulate** (`-- --format raw`). Wire `ORACLE_SOURCE` / `PYTH_API_KEY` (or `--oracle-source`, default `waterx_rule`); a ticker no listed source serves is reported `OK (STALE — no source served this ticker)` |
